...
const WORKER_ID = "briefing";
const JOB_ID = "daily-briefing";
const SETTINGS_KEY = 'settings';
const HISTORY_KEY = 'history';
const LLM_TIMEOUT_MS = 120_000;
const SOURCE_CHAR_LIMIT = 4_000;
const DEFAULT_PROMPT = "You create an executive daily briefing from calendar, inbox, and message context. Prioritize what needs attention today, conflicts, urgent replies, and preparation notes. Keep the output concrete, calm, and short.";
...
const SettingsSchema = z.object({
  ...
  bearerTokenEnv: z.string().default("GOOGLE_ACCESS_TOKEN,SLACK_BOT_TOKEN"),
...
function fallbackResult(records: Array<{ source: string; body: string }>, params: JobParams, errors: Array<{ source: string; message: string }>): LlmResult {
  ...
  const summary = records.length === 0
    ? "Daily Briefing" + ' needs at least one source endpoint or context note before it can produce a report.'
    : "Daily Briefing" + ' processed ' + records.length + ' source' + (records.length === 1 ? '' : 's') + ' without an AI provider.';
...
async function runAutomation(modelId: string, params: JobParams): Promise<{ summary: string; itemCount: number }> {
  ...
  if (settings.publishItems && result.items.length > 0) {
    await publishItem({ producerWorkerId: WORKER_ID, itemType: "briefing.digest", tags: ["briefing","calendar","gmail"].concat([status]), title: "Daily briefing", shortDesc: result.summary, payload: { summary: result.summary, urgentCount: result.urgentCount, items: result.items, status, sourceCount: endpoints.length, fetchedCount: records.length, errors, generatedAt: now }, selectionReason: "Daily Briefing" + ' generated this report from configured sources.' });
    publishedCount = 1;
  }
...
const job: WorkerJobManifest = {
  ...
  description: "Builds a concise summary of calendar, email, and message context from configured source endpoints.",
  defaultEnabled: true,
  defaultCron: "30 14 * * 1-5",
  defaultModelAlias: '',
  ...
  presets: [
    { id: 'quiet-summary', label: 'Quiet summary', description: 'Short report with high-signal items only.', cron: "30 14 * * 1-5", params: { lookbackHours: 24, maxItems: 8, priorityThreshold: 'high' } },
    { id: 'full-sweep', label: 'Full sweep', description: 'Broader scan with low-priority context included.', cron: "30 14 * * 1-5", params: { lookbackHours: 72, maxItems: 20, priorityThreshold: 'low' } },
  ],
...
const manifest: WorkerManifest = {
  ...
  version: '0.1.0',
  description: "Builds a concise summary of calendar, email, and message context from configured source endpoints.",
  tagline: "Turns your calendar, email, and message context into a focused daily briefing.",
  owner: '@ccascio',
  builtIn: false,
  kind: 'feature',
  jobs: [job],
  chatPrompts: [
    { label: 'Latest report', description: 'Ask what the worker found most recently.', prompt: "Show me the latest Daily Briefing report and the highest-priority actions." },
    { label: 'Setup help', description: 'Ask how to connect sources.', prompt: "Help me configure Daily Briefing source endpoints and credentials." },
  ],
  ownedSettings: [
    { key: WORKER_ID + '-config', label: "Daily Briefing sources", description: "Calendar, email, and message endpoints", scope: 'worker', storageKey: 'worker.' + WORKER_ID + '.settings', dashboardTarget: 'config' },
    { key: WORKER_ID + '-job', label: "Daily briefing schedule", description: 'Cron, model, prompt, and run parameters.', scope: 'job', storageKey: 'admin.settings.jobs.' + JOB_ID, dashboardTarget: 'jobs' },
  ],
  dashboard: {
    settings: [{
      id: WORKER_ID + '-config', label: "Daily Briefing sources", description: "Calendar, email, and message endpoints", tab: 'config', path: '/api/workers/' + WORKER_ID + '/settings',
      fields: [
        { key: 'sourceEndpoints', label: "Calendar, email, and message endpoints", type: 'textarea', defaultValue: '', rows: 8, placeholder: "https://www.googleapis.com/calendar/v3/calendars/primary/events\nhttps://gmail.googleapis.com/gmail/v1/users/me/messages\nhttps://slack.com/api/conversations.history?channel=C012345", helpText: 'One HTTP(S) endpoint per line. Endpoints can be native APIs, internal exports, webhooks, or JSON/text snapshots.', seedPath: WORKER_ID + '.settings.sourceEndpoints' },
        { key: 'bearerTokenEnv', label: 'Bearer token env vars', type: 'text', defaultValue: "GOOGLE_ACCESS_TOKEN,SLACK_BOT_TOKEN", placeholder: "GOOGLE_ACCESS_TOKEN,SLACK_BOT_TOKEN", helpText: 'Comma-separated environment variable names. The first variable with a value is used as a Bearer token.', seedPath: WORKER_ID + '.settings.bearerTokenEnv' },
        { key: 'contextNotes', label: 'Context notes', type: 'textarea', defaultValue: '', rows: 6, placeholder: "My working hours are 09:00-18:00 Europe/Rome. Highlight meetings needing preparation and unanswered urgent messages.", helpText: 'Optional instructions, priorities, account names, or pasted context to include in every run.', seedPath: WORKER_ID + '.settings.contextNotes' },
        { key: 'publishItems', label: 'Publish report to Item Bus', type: 'boolean', defaultValue: true, helpText: "When enabled, each run publishes the report as a briefing.digest item." },
      ],
    }],
    routes: [{ id: WORKER_ID + '-dashboard', label: "Briefing", description: "Builds a concise summary of calendar, email, and message context from configured source endpoints.", tab: 'worker', path: '/api/workers/' + WORKER_ID + '/dashboard' }],
  },
};
