...
function WorkerDashboard(ctx: any) {
  ...
  const slice = ctx.dashboard?.workerData?.["issue-triage"] ?? {};
  ...
  const job = ctx.dashboard?.cron?.jobs?.find((entry: any) => entry.name === "issue-triage" || entry.id === "issue-triage");
  ...
  return (
    <>
      <section className="grid top-grid tab-page">
        <article className="panel">
          <div className="panel-head">
            <div><p className="panel-kicker">Issue Triage</p><h2>{sourceCount > 0 || settings.contextNotes ? 'Ready to run' : 'Configure sources'}</h2></div>
          ...
          <div className="detail-body">
            <div className="detail-grid">
              ...
              <Detail label="Cron" value={job?.cron ?? "30 17 * * 1-5"} />
              <Detail label="Token" value={tokenConfigured ? 'configured' : 'not detected'} />
              <Detail label="Publishes" value="issue.triage" />
          ...
          <div className="panel-actions">
            <button type="button" disabled={ctx.busyKey === 'run-issue-triage' || job?.running} onClick={() => ctx.triggerRun?.('run-issue-triage', '/api/cron-jobs/issue-triage/run', 'Issue triage started.')}>
      ...
      <section className="panel tab-page">
        <div className="panel-head"><div><p className="panel-kicker">Output</p><h2>Issue triage report</h2></div><StatusPill tone="muted">{items.length} items</StatusPill></div>
        <div className="stack-list compact">
          ...
          {items.length === 0 ? <p className="empty-state">No triaged issues have been produced yet.</p> : null}
        </div>
      </section>
      ...
      <details className="panel tab-page worker-help-footer">
        <summary>About Issue Triage</summary>
        <div className="detail-body">
          <p><strong>What it does</strong></p><p>Reviews incoming issues, bugs, and feature requests and recommends labels, priority, and ownership.</p>
          ...
          <p><strong>Inputs / outputs</strong></p><p>Reads Linear, GitHub Issues, or compatible issue export/API endpoints. Produces <code>issue.triage</code> items when publishing is enabled.</p>
          <p><strong>Example setup</strong></p><pre style={{ fontSize: '0.8rem', background: 'var(--surface-2, #f5f5f5)', padding: '0.5rem', borderRadius: '4px', whiteSpace: 'pre-wrap' }}>https://api.linear.app/graphql
https://api.github.com/repos/OWNER/REPO/issues?state=open&since=YYYY-MM-DDTHH:MM:SSZ</pre>
...
window.bfrost.registerDashboardView({
  workerId: "issue-triage",
  kind: 'worker-dashboard',
  surfaceIds: ["issue-triage-dashboard"],
  menu: { icon: "list-checks", group: 'Workers', order: 60, label: "Issues" },
  count: (ctx: any) => {
    const items = ctx.dashboard?.workerData?.["issue-triage"]?.lastRun?.items ?? [];
