...
function WorkerDashboard(ctx: any) {
  ...
  const slice = ctx.dashboard?.workerData?.["email-triage"] ?? {};
  ...
  const job = ctx.dashboard?.cron?.jobs?.find((entry: any) => entry.name === "email-triage" || entry.id === "email-triage");
  ...
  return (
    <>
      <section className="grid top-grid tab-page">
        <article className="panel">
          <div className="panel-head">
            <div><p className="panel-kicker">Email Triage</p><h2>{sourceCount > 0 || settings.contextNotes ? 'Ready to run' : 'Configure sources'}</h2></div>
          ...
          <div className="detail-body">
            <div className="detail-grid">
              ...
              <Detail label="Cron" value={job?.cron ?? "0 17 * * 1-5"} />
              <Detail label="Token" value={tokenConfigured ? 'configured' : 'not detected'} />
              <Detail label="Publishes" value="email.triage" />
          ...
          <div className="panel-actions">
            <button type="button" disabled={ctx.busyKey === 'run-email-triage' || job?.running} onClick={() => ctx.triggerRun?.('run-email-triage', '/api/cron-jobs/email-triage/run', 'Email triage started.')}>
      ...
      <section className="panel tab-page">
        <div className="panel-head"><div><p className="panel-kicker">Output</p><h2>Inbox triage</h2></div><StatusPill tone="muted">{items.length} items</StatusPill></div>
        <div className="stack-list compact">
          ...
          {items.length === 0 ? <p className="empty-state">No triaged emails have been produced yet.</p> : null}
        </div>
      </section>
      ...
      <details className="panel tab-page worker-help-footer">
        <summary>About Email Triage</summary>
        <div className="detail-body">
          <p><strong>What it does</strong></p><p>Categorizes and prioritizes inbox context, with draft responses for urgent items.</p>
          ...
          <p><strong>Inputs / outputs</strong></p><p>Reads Gmail or compatible inbox export/API endpoints. Produces <code>email.triage</code> items when publishing is enabled.</p>
          <p><strong>Example setup</strong></p><pre style={{ fontSize: '0.8rem', background: 'var(--surface-2, #f5f5f5)', padding: '0.5rem', borderRadius: '4px', whiteSpace: 'pre-wrap' }}>https://gmail.googleapis.com/gmail/v1/users/me/messages?q=newer_than:2d
https://api.example.com/inbox/export.json</pre>
...
window.bfrost.registerDashboardView({
  workerId: "email-triage",
  kind: 'worker-dashboard',
  surfaceIds: ["email-triage-dashboard"],
  menu: { icon: "mail", group: 'Workers', order: 60, label: "Email" },
  count: (ctx: any) => {
    const items = ctx.dashboard?.workerData?.["email-triage"]?.lastRun?.items ?? [];
