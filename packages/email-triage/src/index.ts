...
const WORKER_ID = "email-triage";
const JOB_ID = "email-triage";
const SETTINGS_KEY = 'settings';
const HISTORY_KEY = 'history';
const LLM_TIMEOUT_MS = 120_000;
const SOURCE_CHAR_LIMIT = 4_000;
const DEFAULT_PROMPT = "You are an inbox triage assistant. Classify email-like items by urgency, explain why, and draft concise replies only when a response is warranted. Do not invent facts not present in the source material.";
...
const SettingsSchema = z.object({
  ...
  bearerTokenEnv: z.string().default("GMAIL_ACCESS_TOKEN"),
...
function fallbackResult(records: Array<{ source: string; body: string }>, params: JobParams, errors: Array<{ source: string; message: string }>): LlmResult {
  ...
  const summary = records.length === 0
    ? "Email Triage" + ' needs at least one source endpoint or context note before it can produce a report.'
    : "Email Triage" + ' processed ' + records.length + ' source' + (records.length === 1 ? '' : 's') + ' without an AI provider.';
...
async function runAutomation(modelId: string, params: JobParams): Promise<{ summary: string; itemCount: number }> {
  ...
  if (settings.publishItems && result.items.length > 0) {
    await publishItem({ producerWorkerId: WORKER_ID, itemType: "email.triage", tags: ["email","gmail","triage"].concat([status]), title: "Inbox triage", shortDesc: result.summary, payload: { summary: result.summary, urgentCount: result.urgentCount, items: result.items, status, sourceCount: endpoints.length, fetchedCount: records.length, errors, generatedAt: now }, selectionReason: "Email Triage" + ' generated this report from configured sources.' });
    publishedCount = 1;
  }
...
const job: WorkerJobManifest = {
  ...
  workerId: WORKER_ID,
  label: "Email triage",
  description: "Categorizes and prioritizes inbox context, with draft responses for urgent items.",
  defaultEnabled: true,
  ...
  presets: [
    { id: 'quiet-summary', label: 'Quiet summary', description: 'Short report with high-signal items only.', cron: "0 17 * * 1-5", params: { lookbackHours: 24, maxItems: 8, priorityThreshold: 'high' } },
    { id: 'full-sweep', label: 'Full sweep', description: 'Broader scan with low-priority context included.', cron: "0 17 * * 1-5", params: { lookbackHours: 72, maxItems: 20, priorityThreshold: 'low' } },
  ],
...
const manifest: WorkerManifest = {
  ...
  version: '0.1.0',
  description: "Categorizes and prioritizes inbox context, with draft responses for urgent items.",
  tagline: "Sorts inbox items by urgency and drafts replies for messages that need a fast response.",
  owner: '@ccascio',
  builtIn: false,
  kind: 'feature',
  jobs: [job],
  chatPrompts: [
    { label: 'Latest report', description: 'Ask what the worker found most recently.', prompt: "Show me the latest Email Triage report and the highest-priority actions." },
    { label: 'Setup help', description: 'Ask how to connect sources.', prompt: "Help me configure Email Triage source endpoints and credentials." },
  ],
  ownedSettings: [
    { key: WORKER_ID + '-config', label: "Email Triage sources", description: "Inbox endpoints", scope: 'worker', storageKey: 'worker.' + WORKER_ID + '.settings', dashboardTarget: 'config' },
    { key: WORKER_ID + '-job', label: "Email triage schedule", description: 'Cron, model, prompt, and run parameters.', scope: 'job', storageKey: 'admin.settings.jobs.' + JOB_ID, dashboardTarget: 'jobs' },
  ],
  dashboard: {
    settings: [{
      id: WORKER_ID + '-config', label: "Email Triage sources", description: "Inbox endpoints", tab: 'config', path: '/api/workers/' + WORKER_ID + '/settings',
      fields: [
        { key: 'sourceEndpoints', label: "Inbox endpoints", type: 'textarea', defaultValue: '', rows: 8, placeholder: "https://gmail.googleapis.com/gmail/v1/users/me/messages?q=newer_than:2d\nhttps://api.example.com/inbox/export.json", helpText: 'One HTTP(S) endpoint per line. Endpoints can be native APIs, internal exports, webhooks, or JSON/text snapshots.', seedPath: WORKER_ID + '.settings.sourceEndpoints' },
        { key: 'bearerTokenEnv', label: 'Bearer token env vars', type: 'text', defaultValue: "GMAIL_ACCESS_TOKEN", placeholder: "GMAIL_ACCESS_TOKEN", helpText: 'Comma-separated environment variable names. The first variable with a value is used as a Bearer token.', seedPath: WORKER_ID + '.settings.bearerTokenEnv' },
        { key: 'contextNotes', label: 'Context notes', type: 'textarea', defaultValue: '', rows: 6, placeholder: "Prioritize customer issues, finance/legal requests, and anything blocking today. Draft replies in my concise style.", helpText: 'Optional instructions, priorities, account names, or pasted context to include in every run.', seedPath: WORKER_ID + '.settings.contextNotes' },
        { key: 'publishItems', label: 'Publish report to Item Bus', type: 'boolean', defaultValue: true, helpText: "When enabled, each run publishes the report as a email.triage item." },
      ],
    }],
    routes: [{ id: WORKER_ID + '-dashboard', label: "Email", description: "Categorizes and prioritizes inbox context, with draft responses for urgent items.", tab: 'worker', path: '/api/workers/' + WORKER_ID + '/dashboard' }],
  },
};
