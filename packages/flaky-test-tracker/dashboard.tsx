...
function WorkerDashboard(ctx: any) {
  ...
  const slice = ctx.dashboard?.workerData?.["flaky-test-tracker"] ?? {};
  ...
  const job = ctx.dashboard?.cron?.jobs?.find((entry: any) => entry.name === "flaky-test-tracker" || entry.id === "flaky-test-tracker");
  ...
  return (
    <>
      <section className="grid top-grid tab-page">
        <article className="panel">
          <div className="panel-head">
            <div><p className="panel-kicker">Flaky Test Tracker</p><h2>{sourceCount > 0 || settings.contextNotes ? 'Ready to run' : 'Configure sources'}</h2></div>
          ...
          <div className="detail-body">
            <div className="detail-grid">
              ...
              <Detail label="Cron" value={job?.cron ?? "0 18 * * 1"} />
              <Detail label="Token" value={tokenConfigured ? 'configured' : 'not detected'} />
              <Detail label="Publishes" value="dev.flaky-test-report" />
          ...
          <div className="panel-actions">
            <button type="button" disabled={ctx.busyKey === 'run-flaky-test-tracker' || job?.running} onClick={() => ctx.triggerRun?.('run-flaky-test-tracker', '/api/cron-jobs/flaky-test-tracker/run', 'Flaky test tracker started.')}>
      ...
      <section className="panel tab-page">
        <div className="panel-head"><div><p className="panel-kicker">Output</p><h2>Flaky test report</h2></div><StatusPill tone="muted">{items.length} items</StatusPill></div>
        <div className="stack-list compact">
          ...
          {items.length === 0 ? <p className="empty-state">No flaky test signals have been produced yet.</p> : null}
        </div>
      </section>
      ...
      <details className="panel tab-page worker-help-footer">
        <summary>About Flaky Test Tracker</summary>
        <div className="detail-body">
          <p><strong>What it does</strong></p><p>Tracks intermittent test failures across recent CI runs and highlights likely flaky tests.</p>
          ...
          <p><strong>Inputs / outputs</strong></p><p>Reads GitHub Actions, CI exports, JUnit summaries, or compatible test-report endpoints. Produces <code>dev.flaky-test-report</code> items when publishing is enabled.</p>
          <p><strong>Example setup</strong></p><pre style={{ fontSize: '0.8rem', background: 'var(--surface-2, #f5f5f5)', padding: '0.5rem', borderRadius: '4px', whiteSpace: 'pre-wrap' }}>https://api.github.com/repos/OWNER/REPO/actions/runs?per_page=20
https://api.github.com/repos/OWNER/REPO/actions/runs/RUN_ID/jobs</pre>
...
window.bfrost.registerDashboardView({
  workerId: "flaky-test-tracker",
  kind: 'worker-dashboard',
  surfaceIds: ["flaky-test-tracker-dashboard"],
  menu: { icon: "flask-conical", group: 'Workers', order: 60, label: "Flaky Tests" },
  count: (ctx: any) => {
    const items = ctx.dashboard?.workerData?.["flaky-test-tracker"]?.lastRun?.items ?? [];
