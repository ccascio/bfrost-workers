...
const WORKER_ID = "flaky-test-tracker";
const JOB_ID = "flaky-test-tracker";
const SETTINGS_KEY = 'settings';
const HISTORY_KEY = 'history';
const LLM_TIMEOUT_MS = 120_000;
const SOURCE_CHAR_LIMIT = 4_000;
const DEFAULT_PROMPT = "You are a test reliability analyst. Compare recent CI/test data and identify likely flaky tests, repeated failure signatures, affected workflows, and recommended stabilization steps.";
...
function fallbackResult(records: Array<{ source: string; body: string }>, params: JobParams, errors: Array<{ source: string; message: string }>): LlmResult {
  ...
  const summary = records.length === 0
    ? "Flaky Test Tracker" + ' needs at least one source endpoint or context note before it can produce a report.'
    : "Flaky Test Tracker" + ' processed ' + records.length + ' source' + (records.length === 1 ? '' : 's') + ' without an AI provider.';
...
async function runAutomation(modelId: string, params: JobParams): Promise<{ summary: string; itemCount: number }> {
  ...
  if (settings.publishItems && result.items.length > 0) {
    await publishItem({ producerWorkerId: WORKER_ID, itemType: "dev.flaky-test-report", tags: ["ci","tests","flaky-tests"].concat([status]), title: "Flaky test report", shortDesc: result.summary, payload: { summary: result.summary, urgentCount: result.urgentCount, items: result.items, status, sourceCount: endpoints.length, fetchedCount: records.length, errors, generatedAt: now }, selectionReason: "Flaky Test Tracker" + ' generated this report from configured sources.' });
    publishedCount = 1;
  }
...
const job: WorkerJobManifest = {
  id: JOB_ID,
  workerId: WORKER_ID,
  label: "Flaky test tracker",
  description: "Tracks intermittent test failures across recent CI runs and highlights likely flaky tests.",
  defaultEnabled: true,
  defaultCron: "0 18 * * 1",
  defaultModelAlias: '',
  ...
  presets: [
    { id: 'quiet-summary', label: 'Quiet summary', description: 'Short report with high-signal items only.', cron: "0 18 * * 1", params: { lookbackHours: 24, maxItems: 8, priorityThreshold: 'high' } },
    { id: 'full-sweep', label: 'Full sweep', description: 'Broader scan with low-priority context included.', cron: "0 18 * * 1", params: { lookbackHours: 72, maxItems: 20, priorityThreshold: 'low' } },
  ],
...
const manifest: WorkerManifest = {
  ...
  displayName: "Flaky Test Tracker",
  version: '0.1.0',
  description: "Tracks intermittent test failures across recent CI runs and highlights likely flaky tests.",
  tagline: "Finds tests that pass and fail intermittently across recent CI runs.",
  owner: '@ccascio',
  builtIn: false,
  kind: 'feature',
  jobs: [job],
  chatPrompts: [
    { label: 'Latest report', description: 'Ask what the worker found most recently.', prompt: "Show me the latest Flaky Test Tracker report and the highest-priority actions." },
    { label: 'Setup help', description: 'Ask how to connect sources.', prompt: "Help me configure Flaky Test Tracker source endpoints and credentials." },
  ],
  ownedSettings: [
    { key: WORKER_ID + '-config', label: "Flaky Test Tracker sources", description: "CI run and test-report endpoints", scope: 'worker', storageKey: 'worker.' + WORKER_ID + '.settings', dashboardTarget: 'config' },
    { key: WORKER_ID + '-job', label: "Flaky test tracker schedule", description: 'Cron, model, prompt, and run parameters.', scope: 'job', storageKey: 'admin.settings.jobs.' + JOB_ID, dashboardTarget: 'jobs' },
  ],
  dashboard: {
    settings: [{
      id: WORKER_ID + '-config', label: "Flaky Test Tracker sources", description: "CI run and test-report endpoints", tab: 'config', path: '/api/workers/' + WORKER_ID + '/settings',
      fields: [
        { key: 'sourceEndpoints', label: "CI run and test-report endpoints", type: 'textarea', defaultValue: '', rows: 8, placeholder: "https://api.github.com/repos/OWNER/REPO/actions/runs?per_page=20\nhttps://api.github.com/repos/OWNER/REPO/actions/runs/RUN_ID/jobs", helpText: 'One HTTP(S) endpoint per line. Endpoints can be native APIs, internal exports, webhooks, or JSON/text snapshots.', seedPath: WORKER_ID + '.settings.sourceEndpoints' },
        ...
        { key: 'contextNotes', label: 'Context notes', type: 'textarea', defaultValue: '', rows: 6, placeholder: "Flag tests or suites that fail intermittently, especially if later reruns pass. Include workflow, branch, and failure pattern.", helpText: 'Optional instructions, priorities, account names, or pasted context to include in every run.', seedPath: WORKER_ID + '.settings.contextNotes' },
        { key: 'publishItems', label: 'Publish report to Item Bus', type: 'boolean', defaultValue: true, helpText: "When enabled, each run publishes the report as a dev.flaky-test-report item." },
      ],
    }],
    routes: [{ id: WORKER_ID + '-dashboard', label: "Flaky Tests", description: "Tracks intermittent test failures across recent CI runs and highlights likely flaky tests.", tab: 'worker', path: '/api/workers/' + WORKER_ID + '/dashboard' }],
  },
};
