...
const WORKER_ID = "dependency-update-check";
const JOB_ID = "dependency-update-check";
const SETTINGS_KEY = 'settings';
const HISTORY_KEY = 'history';
const LLM_TIMEOUT_MS = 120_000;
const SOURCE_CHAR_LIMIT = 4_000;
const DEFAULT_PROMPT = "You are a dependency update reviewer. Identify security patches, outdated packages, breaking-change risk, recommended update order, and what can wait. Be practical and risk-aware.";
...
const SettingsSchema = z.object({
  ...
  bearerTokenEnv: z.string().default("GITHUB_TOKEN,OSV_API_KEY"),
...
function fallbackResult(records: Array<{ source: string; body: string }>, params: JobParams, errors: Array<{ source: string; message: string }>): LlmResult {
  const items = records.slice(0, params.maxItems).map((record, index) => ({
    ...
    category: "Dependency update report",
  ...
  const summary = records.length === 0
    ? "Dependency Update Check" + ' needs at least one source endpoint or context note before it can produce a report.'
    : "Dependency Update Check" + ' processed ' + records.length + ' source' + (records.length === 1 ? '' : 's') + ' without an AI provider.';
...
async function runAutomation(modelId: string, params: JobParams): Promise<{ summary: string; itemCount: number }> {
  ...
  if (settings.publishItems && result.items.length > 0) {
    await publishItem({ producerWorkerId: WORKER_ID, itemType: "dev.dependency-report", tags: ["dependencies","security","dependabot"].concat([status]), title: "Dependency update report", shortDesc: result.summary, payload: { summary: result.summary, urgentCount: result.urgentCount, items: result.items, status, sourceCount: endpoints.length, fetchedCount: records.length, errors, generatedAt: now }, selectionReason: "Dependency Update Check" + ' generated this report from configured sources.' });
    publishedCount = 1;
  }
...
const job: WorkerJobManifest = {
  ...
  description: "Scans configured dependency/security endpoints for outdated packages, patches, and breaking-change risks.",
  defaultEnabled: true,
  defaultCron: "30 20 * * 1",
  defaultModelAlias: '',
  ...
  presets: [
    { id: 'quiet-summary', label: 'Quiet summary', description: 'Short report with high-signal items only.', cron: "30 20 * * 1", params: { lookbackHours: 24, maxItems: 8, priorityThreshold: 'high' } },
    { id: 'full-sweep', label: 'Full sweep', description: 'Broader scan with low-priority context included.', cron: "30 20 * * 1", params: { lookbackHours: 72, maxItems: 20, priorityThreshold: 'low' } },
  ],
...
const manifest: WorkerManifest = {
  ...
  version: '0.1.0',
  description: "Scans configured dependency/security endpoints for outdated packages, patches, and breaking-change risks.",
  tagline: "Reviews dependency update feeds for security patches, outdated packages, and risky upgrades.",
  owner: '@ccascio',
  builtIn: false,
  kind: 'feature',
  jobs: [job],
  chatPrompts: [
    { label: 'Latest report', description: 'Ask what the worker found most recently.', prompt: "Show me the latest Dependency Update Check report and the highest-priority actions." },
    { label: 'Setup help', description: 'Ask how to connect sources.', prompt: "Help me configure Dependency Update Check source endpoints and credentials." },
  ],
  ownedSettings: [
    { key: WORKER_ID + '-config', label: "Dependency Update Check sources", description: "Dependency and advisory endpoints", scope: 'worker', storageKey: 'worker.' + WORKER_ID + '.settings', dashboardTarget: 'config' },
    { key: WORKER_ID + '-job', label: "Dependency update check schedule", description: 'Cron, model, prompt, and run parameters.', scope: 'job', storageKey: 'admin.settings.jobs.' + JOB_ID, dashboardTarget: 'jobs' },
  ],
  dashboard: {
    settings: [{
      id: WORKER_ID + '-config', label: "Dependency Update Check sources", description: "Dependency and advisory endpoints", tab: 'config', path: '/api/workers/' + WORKER_ID + '/settings',
      fields: [
        { key: 'sourceEndpoints', label: "Dependency and advisory endpoints", type: 'textarea', defaultValue: '', rows: 8, placeholder: "https://api.github.com/repos/OWNER/REPO/dependabot/alerts\nhttps://api.osv.dev/v1/querybatch\nhttps://registry.npmjs.org/PACKAGE", helpText: 'One HTTP(S) endpoint per line. Endpoints can be native APIs, internal exports, webhooks, or JSON/text snapshots.', seedPath: WORKER_ID + '.settings.sourceEndpoints' },
        { key: 'bearerTokenEnv', label: 'Bearer token env vars', type: 'text', defaultValue: "GITHUB_TOKEN,OSV_API_KEY", placeholder: "GITHUB_TOKEN,OSV_API_KEY", helpText: 'Comma-separated environment variable names. The first variable with a value is used as a Bearer token.', seedPath: WORKER_ID + '.settings.bearerTokenEnv' },
        { key: 'contextNotes', label: 'Context notes', type: 'textarea', defaultValue: '', rows: 6, placeholder: "Prioritize security updates, runtime dependencies, and packages used in production deploy paths. Flag likely breaking changes.", helpText: 'Optional instructions, priorities, account names, or pasted context to include in every run.', seedPath: WORKER_ID + '.settings.contextNotes' },
        { key: 'publishItems', label: 'Publish report to Item Bus', type: 'boolean', defaultValue: true, helpText: "When enabled, each run publishes the report as a dev.dependency-report item." },
      ],
    }],
    routes: [{ id: WORKER_ID + '-dashboard', label: "Deps", description: "Scans configured dependency/security endpoints for outdated packages, patches, and breaking-change risks.", tab: 'worker', path: '/api/workers/' + WORKER_ID + '/dashboard' }],
  },
};
