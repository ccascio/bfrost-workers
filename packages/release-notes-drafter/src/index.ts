...
const WORKER_ID = "release-notes-drafter";
const JOB_ID = "release-notes-draft";
const SETTINGS_KEY = 'settings';
const HISTORY_KEY = 'history';
const LLM_TIMEOUT_MS = 120_000;
const SOURCE_CHAR_LIMIT = 4_000;
const DEFAULT_PROMPT = "You are a release notes editor. Turn merged PR and changelog material into concise user-facing notes. Group changes, remove implementation noise, and preserve caveats or migration notes.";
...
function fallbackResult(records: Array<{ source: string; body: string }>, params: JobParams, errors: Array<{ source: string; message: string }>): LlmResult {
  ...
  const summary = records.length === 0
    ? "Release Notes Drafter" + ' needs at least one source endpoint or context note before it can produce a report.'
    : "Release Notes Drafter" + ' processed ' + records.length + ' source' + (records.length === 1 ? '' : 's') + ' without an AI provider.';
...
async function runAutomation(modelId: string, params: JobParams): Promise<{ summary: string; itemCount: number }> {
  ...
  if (settings.publishItems && result.items.length > 0) {
    await publishItem({ producerWorkerId: WORKER_ID, itemType: "release.notes-draft", tags: ["release-notes","github","pull-requests"].concat([status]), title: "Release notes draft", shortDesc: result.summary, payload: { summary: result.summary, urgentCount: result.urgentCount, items: result.items, status, sourceCount: endpoints.length, fetchedCount: records.length, errors, generatedAt: now }, selectionReason: "Release Notes Drafter" + ' generated this report from configured sources.' });
    publishedCount = 1;
  }
...
const job: WorkerJobManifest = {
  ...
  description: "Drafts user-facing release notes from merged PR or changelog source data.",
  defaultEnabled: true,
  defaultCron: "*/30 * * * *",
  defaultModelAlias: '',
  ...
  presets: [
    { id: 'quiet-summary', label: 'Quiet summary', description: 'Short report with high-signal items only.', cron: "*/30 * * * *", params: { lookbackHours: 24, maxItems: 8, priorityThreshold: 'high' } },
    { id: 'full-sweep', label: 'Full sweep', description: 'Broader scan with low-priority context included.', cron: "*/30 * * * *", params: { lookbackHours: 72, maxItems: 20, priorityThreshold: 'low' } },
  ],
...
const manifest: WorkerManifest = {
  ...
  version: '0.1.0',
  description: "Drafts user-facing release notes from merged PR or changelog source data.",
  tagline: "Turns merged PRs and changelog entries into clear user-facing release notes.",
  owner: '@ccascio',
  builtIn: false,
  kind: 'feature',
  jobs: [job],
  chatPrompts: [
    { label: 'Latest report', description: 'Ask what the worker found most recently.', prompt: "Show me the latest Release Notes Drafter report and the highest-priority actions." },
    { label: 'Setup help', description: 'Ask how to connect sources.', prompt: "Help me configure Release Notes Drafter source endpoints and credentials." },
  ],
  ownedSettings: [
    { key: WORKER_ID + '-config', label: "Release Notes Drafter sources", description: "Merged PR and changelog endpoints", scope: 'worker', storageKey: 'worker.' + WORKER_ID + '.settings', dashboardTarget: 'config' },
    { key: WORKER_ID + '-job', label: "Release notes draft schedule", description: 'Cron, model, prompt, and run parameters.', scope: 'job', storageKey: 'admin.settings.jobs.' + JOB_ID, dashboardTarget: 'jobs' },
  ],
  dashboard: {
    settings: [{
      id: WORKER_ID + '-config', label: "Release Notes Drafter sources", description: "Merged PR and changelog endpoints", tab: 'config', path: '/api/workers/' + WORKER_ID + '/settings',
      fields: [
        { key: 'sourceEndpoints', label: "Merged PR and changelog endpoints", type: 'textarea', defaultValue: '', rows: 8, placeholder: "https://api.github.com/repos/OWNER/REPO/pulls?state=closed&sort=updated&direction=desc\nhttps://api.github.com/repos/OWNER/REPO/releases/latest", helpText: 'One HTTP(S) endpoint per line. Endpoints can be native APIs, internal exports, webhooks, or JSON/text snapshots.', seedPath: WORKER_ID + '.settings.sourceEndpoints' },
        ...
        { key: 'contextNotes', label: 'Context notes', type: 'textarea', defaultValue: '', rows: 6, placeholder: "Audience: product users, not engineers. Group by Added, Improved, Fixed, and Internal only when relevant.", helpText: 'Optional instructions, priorities, account names, or pasted context to include in every run.', seedPath: WORKER_ID + '.settings.contextNotes' },
        { key: 'publishItems', label: 'Publish report to Item Bus', type: 'boolean', defaultValue: true, helpText: "When enabled, each run publishes the report as a release.notes-draft item." },
      ],
    }],
    routes: [{ id: WORKER_ID + '-dashboard', label: "Releases", description: "Drafts user-facing release notes from merged PR or changelog source data.", tab: 'worker', path: '/api/workers/' + WORKER_ID + '/dashboard' }],
  },
};
