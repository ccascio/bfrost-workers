...
function WorkerDashboard(ctx: any) {
  ...
  const slice = ctx.dashboard?.workerData?.["release-notes-drafter"] ?? {};
  ...
  const job = ctx.dashboard?.cron?.jobs?.find((entry: any) => entry.name === "release-notes-draft" || entry.id === "release-notes-draft");
  ...
  return (
    <>
      <section className="grid top-grid tab-page">
        <article className="panel">
          <div className="panel-head">
            <div><p className="panel-kicker">Release Notes Drafter</p><h2>{sourceCount > 0 || settings.contextNotes ? 'Ready to run' : 'Configure sources'}</h2></div>
          ...
          <div className="detail-body">
            <div className="detail-grid">
              ...
              <Detail label="Cron" value={job?.cron ?? "*/30 * * * *"} />
              <Detail label="Token" value={tokenConfigured ? 'configured' : 'not detected'} />
              <Detail label="Publishes" value="release.notes-draft" />
          ...
          <div className="panel-actions">
            <button type="button" disabled={ctx.busyKey === 'run-release-notes-draft' || job?.running} onClick={() => ctx.triggerRun?.('run-release-notes-draft', '/api/cron-jobs/release-notes-draft/run', 'Release notes draft started.')}>
      ...
      <section className="panel tab-page">
        <div className="panel-head"><div><p className="panel-kicker">Output</p><h2>Release notes draft</h2></div><StatusPill tone="muted">{items.length} items</StatusPill></div>
        <div className="stack-list compact">
          ...
          {items.length === 0 ? <p className="empty-state">No release notes have been produced yet.</p> : null}
        </div>
      </section>
      ...
      <details className="panel tab-page worker-help-footer">
        <summary>About Release Notes Drafter</summary>
        <div className="detail-body">
          <p><strong>What it does</strong></p><p>Drafts user-facing release notes from merged PR or changelog source data.</p>
          ...
          <p><strong>Inputs / outputs</strong></p><p>Reads GitHub merged PRs, changelog exports, or release feed endpoints. Produces <code>release.notes-draft</code> items when publishing is enabled.</p>
          <p><strong>Example setup</strong></p><pre style={{ fontSize: '0.8rem', background: 'var(--surface-2, #f5f5f5)', padding: '0.5rem', borderRadius: '4px', whiteSpace: 'pre-wrap' }}>https://api.github.com/repos/OWNER/REPO/pulls?state=closed&sort=updated&direction=desc
https://api.github.com/repos/OWNER/REPO/releases/latest</pre>
...
window.bfrost.registerDashboardView({
  workerId: "release-notes-drafter",
  kind: 'worker-dashboard',
  surfaceIds: ["release-notes-drafter-dashboard"],
  menu: { icon: "scroll-text", group: 'Workers', order: 60, label: "Releases" },
  count: (ctx: any) => {
    const items = ctx.dashboard?.workerData?.["release-notes-drafter"]?.lastRun?.items ?? [];
