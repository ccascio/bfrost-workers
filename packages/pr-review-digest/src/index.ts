...
const DEFAULT_PROMPT = "You are a senior engineering lead preparing a pull request review digest. Highlight open PRs, review state, CI failures, merge blockers, stale threads, and the smallest useful next action for each PR.";
...
function fallbackResult(records: Array<{ source: string; body: string }>, params: JobParams, errors: Array<{ source: string; message: string }>): LlmResult {
  ...
  const summary = records.length === 0
    ? "PR Review Digest" + ' needs at least one source endpoint or context note before it can produce a report.'
    : "PR Review Digest" + ' processed ' + records.length + ' source' + (records.length === 1 ? '' : 's') + ' without an AI provider.';
...
async function runAutomation(modelId: string, params: JobParams): Promise<{ summary: string; itemCount: number }> {
  ...
  if (settings.publishItems && result.items.length > 0) {
    await publishItem({ producerWorkerId: WORKER_ID, itemType: "dev.pr-review-digest", tags: ["github","pull-requests","review"].concat([status]), title: "PR review digest", shortDesc: result.summary, payload: { summary: result.summary, urgentCount: result.urgentCount, items: result.items, status, sourceCount: endpoints.length, fetchedCount: records.length, errors, generatedAt: now }, selectionReason: "PR Review Digest" + ' generated this report from configured sources.' });
    publishedCount = 1;
  }
...
const job: WorkerJobManifest = {
  ...
  description: "Summarizes open pull requests, review status, blockers, and what needs attention.",
  defaultEnabled: true,
  ...
  presets: [
    { id: 'quiet-summary', label: 'Quiet summary', description: 'Short report with high-signal items only.', cron: "0 20 * * 1-5", params: { lookbackHours: 24, maxItems: 8, priorityThreshold: 'high' } },
    { id: 'full-sweep', label: 'Full sweep', description: 'Broader scan with low-priority context included.', cron: "0 20 * * 1-5", params: { lookbackHours: 72, maxItems: 20, priorityThreshold: 'low' } },
  ],
...
const manifest: WorkerManifest = {
  ...
  version: '0.1.0',
  description: "Summarizes open pull requests, review status, blockers, and what needs attention.",
  tagline: "Summarizes open PRs, review state, blockers, and the next reviews to do.",
  owner: '@ccascio',
  builtIn: false,
  kind: 'feature',
  jobs: [job],
  chatPrompts: [
    { label: 'Latest report', description: 'Ask what the worker found most recently.', prompt: "Show me the latest PR Review Digest report and the highest-priority actions." },
  ...
  dashboard: {
    ...
    routes: [{ id: WORKER_ID + '-dashboard', label: "PRs", description: "Summarizes open pull requests, review status, blockers, and what needs attention.", tab: 'worker', path: '/api/workers/' + WORKER_ID + '/dashboard' }],
  },
};
